...
brms_beta_args <- list(
  formula = brms::bf(r | trials(n) ~ 1, family = brms::brmsfamily("binomial", link = "identity"), center = FALSE),
...
brms_normal_args <- list(
  formula = brms::bf(y ~ 1, family = brms::brmsfamily("gaussian", link = "identity"), center = FALSE),
  data = data.frame(y = 0),
...
test_that("Normal mixture quantiles are correct for brms sampled prior", {
  mixstanvar_simul_test(normMix, brms_normal_args, eps, c(2, 1), ptest = c(0.3, 0.5, 0.7))
})
...
test_that("Gamma mixture quantile function is correct for brms sampled prior", {
  mixstanvar_simul_test(gammaMix, brms_gamma_args, eps, c(2, 7), ptest = seq(0.2, 0.8, by = 0.2))
})
...
mixstanvar_test_mvnormMix <- function(mix,
                                      brms_args) {
  brms_prior_empty <- do.call(brms::brm, c(brms_args, list(seed = 1423545, refresh = 0, sample_prior = "only", stanvars = mixstanvar(prior = mix), empty = TRUE)))
  ...
  for (i in seq_len(Nc)) {
    ...
    Rho_c <- cov2cor(S_c)
    expect_equal(Rho_c[lower.tri(Rho_c)], unname(mix[(1 + 2 * p + 1):nrow(mix), i]))
  }
}
...
brms_mvn_4_args <- list(
  formula = brms::bf(y ~ 1 + l1 + l2 + l3, family = brms::brmsfamily("gaussian", link = "identity"), center = FALSE),
...
brms_mvn_2_args <- list(
  formula = brms::bf(y ~ 1 + l1, family = brms::brmsfamily("gaussian", link = "identity"), center = FALSE),
