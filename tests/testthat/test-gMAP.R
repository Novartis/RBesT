...
test_that("gMAP processes not continuously labeled studies", {
  suppressWarnings(out <- capture.output(map1 <- gMAP(cbind(r, n - r) ~ 1 | study,
    data = AS[-1, ],
...
test_that("gMAP reports divergences", {
  suppressMessages(suppressWarnings(mcmc_div <- gMAP(cbind(r, n - r) ~ 1 | study,
    data = AS[1, , drop = FALSE], family = binomial,
...
test_that("gMAP handles extreme response rates", {
  ...
  suppressWarnings(map1 <- gMAP(cbind(r, n - r) ~ 1 | study,
    family = binomial,
  ...
  suppressWarnings(map2 <- gMAP(cbind(r, n - r) ~ 1 | study,
    family = binomial,
  ...
  suppressWarnings(map3 <- gMAP(cbind(r, n - r) ~ 1 | study,
    family = binomial,
...
test_that("gMAP labels data rows correctly when using covariates", {
  ...
  suppressMessages(suppressWarnings(map_covs <- gMAP(cbind(r, n - r) ~ 1 + stratum | study,
    family = binomial,
  ...
  suppressMessages(suppressWarnings(map_tau_strata <- gMAP(cbind(r, n - r) ~ 1 | id,
    family = binomial,
    tau.strata = stratum,
    data = data_covs, tau.dist = "Fixed",
